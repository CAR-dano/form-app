...
class _MultiShotCameraScreenState extends ConsumerState<MultiShotCameraScreen>
  ...
  Future<void> _setupCameras() async {
    try {
      ...
      ref.read(crashlyticsUtilProvider).recordError(e, stackTrace, reason: 'Error discovering cameras', fatal: false);
      if (mounted) {
        ref.read(customMessageOverlayProvider).show(
          context: context, // Pass context here
  ...
  Future<void> _onTakePicture() async {
    ...
    try {
      ...
      ref.read(crashlyticsUtilProvider).recordError(e, stackTrace, reason: 'Error taking picture', fatal: false);
      if (mounted) {
        ref.read(customMessageOverlayProvider).show(
          context: context, // Pass context here
